    ...
    <aside className="w-64 flex-shrink-0 bg-surface-1 border-r border-border flex flex-col h-full">
      ...
      <div className="p-3 border-b border-border">
        ...
        <div className="flex items-center gap-2 px-1 mb-3">
          ...
          <span className="font-semibold text-gray-100 text-sm">Ollama Chat</span>
        </div>

        {/* Model Picker */}
        <div className="relative mb-3">
          <button onClick={() => setShowModelPicker(p => !p)}
            className="w-full flex items-center gap-2 px-3 py-2 bg-surface-3 hover:bg-surface-4
            ...
            <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${showModelPicker ? 'rotate-180' : ''}`} />
          </button>

          {showModelPicker && models.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-surface-2 border border-border
                            rounded-lg shadow-2xl z-50 overflow-hidden max-h-56 overflow-y-auto">
              {models.map(m => (
                <button key={m.name} onClick={() => handleSelectModel(m.name)}
                  className={`w-full px-3 py-2.5 text-left text-xs hover:bg-surface-3 transition-colors
                    ${m.name === selectedModel ? 'text-accent bg-accent/10 font-medium' : 'text-gray-300'}`}>
                  <div className="font-medium">{m.name}</div>
                  {m.details?.parameter_size && (
                    <div className="text-gray-500 text-[10px] mt-0.5">{m.details.parameter_size}</div>
                  )}
                </button>
              ))}
            </div>
          ...
          <button key={chat.id} onClick={() => navigate(`/${chat.id}`)}
            className={`group w-full flex items-center gap-2 px-3 py-2 rounded-lg text-left
                        ...
                        ${String(chatId) === String(chat.id)
                          ...
                          : 'hover:bg-surface-3 text-gray-400 hover:text-gray-100'}`}>
      ...
      <div className="p-2 border-t border-border space-y-0.5">
        ...
        <div className="flex items-center gap-2 px-3 py-2 text-xs text-gray-500
          ...
          <span className="flex-1 truncate text-gray-300">{user?.username}</span>
